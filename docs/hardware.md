...
- **Pop suppression:** Analog switch IC per output pair (DG419 or TS5A3159, SPST)
  ...
  - Tiny footprint (SOT-23-5 for TS5A3159 or SOIC-8 for DG419), low on-resistance (<1Ω)
...
- **P-FET load switch** (e.g., TPS22965 or equivalent, 5A continuous) on main 5V input after polyfuse
...
- **Implementation:** 2× BSS138 N-FETs wired as SR latch (or single 74LVC1G00 NAND gate), RC network for long-press timeout, debounce cap on button input
...
| DG419 or TS5A3159 analog switch | 4      | Pop suppression — 1 per output stereo pair (Main, AUX1-3) |
...
| TPS22965 (or equiv) load switch | 1       | Soft-start / inrush limiting, 5A continuous |
...
- Top-right corner cluster: Power button, PC (USB-C, data only), MIDI HOST (dual USB-A stacked), MIDI IN (3.5mm TRS Type A)
